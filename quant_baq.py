...
def quantize_vec(x, scale, zero, maxq, x_mean, layer_idx=None, name=None):
    if isinstance(maxq, torch.Tensor):
        scalar_maxq = maxq.item() if maxq.numel() == 1 else None
    else:
        scalar_maxq = maxq
    if scalar_maxq is not None and scalar_maxq < 0:
        return (x > scale / 2).float() * scale + (x < zero / 2).float() * zero
    ...
    zero_tensor = torch.zeros_like(maxq)

    # mask = (maxq == 0)
    q = torch.clamp(torch.round(x / scale) + zero, min=zero_tensor, max=maxq)
    # mask = torch.isnan(q)
    mask = q > 1e3
    q_alt = torch.clamp(torch.round(x_mean / scale) + zero, min=zero_tensor, max=maxq)
    q[mask] = q_alt[mask]
...
class Quantizer(nn.Module):
    ...
    def find_params(self, x, weight=False):
        ...
        if self.maxq < 0:
          ...
          # self.scale = 1.0*(xmax - xmin) / (self.maxq+1)
          if self.sym:
              self.zero = torch.full_like(self.scale, (self.maxq + 1) / 2)
          else:
              self.zero = torch.round(-xmin / self.scale)
              # self.zero = -(xmin-(xmax-xmin)*0)-0.5*self.scale
              
        # print(f"zero shape: {self.zero.shape}, dtype: {self.zero.dtype}, values: {self.zero}")
        if self.mse:
